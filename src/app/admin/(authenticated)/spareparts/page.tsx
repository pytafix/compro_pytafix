...
export default function AdminSpareparts() {
  ...
  const fetchSpareparts = async (isRefresh = false) => {
    ...
    try {
      const res = await fetch("/api/admin/spareparts");
      if (res.ok) {
        setSpareparts(await res.json());
      }
    } catch (error) {
      toast.error("Gagal mengambil data sparepart");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingId ? `/api/admin/spareparts/${editingId}` : "/api/admin/spareparts";
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/admin/spareparts/${id}`, { method: "DELETE" });
      if (res.ok) {
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="font-body-sm text-on-surface-variant truncate max-w-[200px]" title={item.description || ""}>
                            {item.description}
                          </p>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-surface w-full max-w-xl rounded-xl shadow-xl border border-outline-variant overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="px-6 py-4 border-t border-outline-variant bg-surface-container-lowest flex justify-end gap-3">
              ...
              <button type="submit" form="sparepart-form" disabled={isSubmitting} className="px-6 py-2 rounded font-label-bold bg-primary text-on-primary hover:bg-on-primary-fixed-variant transition-colors disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer">
