import Link from "next/link";

export default function Layanan() {
  return (
    <main>
      {/* Hero Section */}
      <section className="bg-surface-container-low py-16 md:py-20 px-4 md:px-8 lg:px-margin-desktop text-center border-b border-outline-variant">
        <div className="max-w-container-max mx-auto">
          <h1 className="font-headline-xl text-headline-xl text-primary mb-4 md:font-headline-xl md:text-headline-xl font-headline-lg-mobile text-headline-lg-mobile">
            Layanan Kami
          </h1>
          <p className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl mx-auto">
            Solusi perbaikan profesional untuk semua perangkat Anda. Teknisi tersertifikasi, suku cadang berkualitas, dan garansi resmi.
          </p>
        </div>
      </section>

      {/* Services Grid */}
      <section className="py-16 md:py-20 px-4 md:px-8 lg:px-margin-desktop bg-background">
        <div className="max-w-container-max mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-gutter">
          {/* Card 1 */}
          <div className="bg-surface border border-outline-variant rounded p-6 hover:border-primary transition-colors flex flex-col h-full group">
            <div className="mb-4 text-primary">
              <span className="material-symbols-outlined text-4xl" data-icon="laptop_mac">
                laptop_mac
              </span>
            </div>
            <h3 className="font-headline-md text-headline-md text-primary mb-2">Service Laptop</h3>
            <p className="font-body-md text-body-md text-on-surface-variant mb-6 flex-grow">
              Perbaikan komprehensif untuk berbagai masalah laptop, mulai dari layar rusak, masalah baterai, hingga optimasi performa sistem.
            </p>
            <div className="flex items-center gap-2 text-secondary bg-secondary-container px-3 py-1 rounded w-fit">
              <span className="material-symbols-outlined text-sm" data-icon="verified_user">
                verified_user
              </span>
              <span className="font-label-bold text-label-bold">Bergaransi</span>
            </div>
          </div>
          {/* Card 2 */}
          <div className="bg-surface border border-outline-variant rounded p-6 hover:border-primary transition-colors flex flex-col h-full group">
            <div className="mb-4 text-primary">
              <span className="material-symbols-outlined text-4xl" data-icon="desktop_windows">
                desktop_windows
              </span>
            </div>
            <h3 className="font-headline-md text-headline-md text-primary mb-2">Service Komputer</h3>
            <p className="font-body-md text-body-md text-on-surface-variant mb-6 flex-grow">
              Diagnosa dan perbaikan PC desktop. Upgrade komponen, pembersihan hardware, dan instalasi sistem operasi.
            </p>
            <div className="flex items-center gap-2 text-secondary bg-secondary-container px-3 py-1 rounded w-fit">
              <span className="material-symbols-outlined text-sm" data-icon="verified_user">
                verified_user
              </span>
              <span className="font-label-bold text-label-bold">Bergaransi</span>
            </div>
          </div>
          {/* Card 3 */}
          <div className="bg-surface border border-outline-variant rounded p-6 hover:border-primary transition-colors flex flex-col h-full group">
            <div className="mb-4 text-primary">
              <span className="material-symbols-outlined text-4xl" data-icon="smartphone">
                smartphone
              </span>
            </div>
            <h3 className="font-headline-md text-headline-md text-primary mb-2">HP & Tablet</h3>
            <p className="font-body-md text-body-md text-on-surface-variant mb-6 flex-grow">
              Penggantian layar, baterai, perbaikan port charger, dan penanganan kerusakan air untuk smartphone dan tablet.
            </p>
            <div className="flex items-center gap-2 text-secondary bg-secondary-container px-3 py-1 rounded w-fit">
              <span className="material-symbols-outlined text-sm" data-icon="verified_user">
                verified_user
              </span>
              <span className="font-label-bold text-label-bold">Bergaransi</span>
            </div>
          </div>
          {/* Card 4 */}
          <div className="bg-surface border border-outline-variant rounded p-6 hover:border-primary transition-colors flex flex-col h-full group">
            <div className="mb-4 text-primary">
              <span className="material-symbols-outlined text-4xl" data-icon="memory">
                memory
              </span>
            </div>
            <h3 className="font-headline-md text-headline-md text-primary mb-2">Sparepart</h3>
            <p className="font-body-md text-body-md text-on-surface-variant mb-6 flex-grow">
              Penyediaan suku cadang original dan berkualitas tinggi untuk berbagai merek dan tipe perangkat elektronik.
            </p>
            <div className="flex items-center gap-2 text-secondary bg-secondary-container px-3 py-1 rounded w-fit">
              <span className="material-symbols-outlined text-sm" data-icon="verified_user">
                verified_user
              </span>
              <span className="font-label-bold text-label-bold">Bergaransi</span>
            </div>
          </div>
          {/* Card 5 */}
          <div className="bg-surface border border-outline-variant rounded p-6 hover:border-primary transition-colors flex flex-col h-full group">
            <div className="mb-4 text-primary">
              <span className="material-symbols-outlined text-4xl" data-icon="home_repair_service">
                home_repair_service
              </span>
            </div>
            <h3 className="font-headline-md text-headline-md text-primary mb-2">Home Service</h3>
            <p className="font-body-md text-body-md text-on-surface-variant mb-6 flex-grow">
              Teknisi kami datang ke lokasi Anda. Solusi praktis untuk perbaikan perangkat tanpa perlu meninggalkan rumah atau kantor.
            </p>
            <div className="flex items-center gap-2 text-secondary bg-secondary-container px-3 py-1 rounded w-fit">
              <span className="material-symbols-outlined text-sm" data-icon="verified_user">
                verified_user
              </span>
              <span className="font-label-bold text-label-bold">Bergaransi</span>
            </div>
          </div>
        </div>
      </section>

      {/* Warranty Policy */}
      <section className="py-16 md:py-20 px-4 md:px-8 lg:px-margin-desktop bg-surface-container border-y border-outline-variant">
        <div className="max-w-container-max mx-auto">
          <div className="text-center mb-12">
            <h2 className="font-headline-lg text-headline-lg text-primary mb-4 md:font-headline-lg md:text-headline-lg font-headline-lg-mobile text-headline-lg-mobile">
              Kebijakan Garansi
            </h2>
            <p className="font-body-lg text-body-lg text-on-surface-variant">
              Komitmen kami terhadap kualitas layanan dan kepuasan pelanggan.
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-gutter">
            <div className="bg-surface p-6 border border-outline-variant rounded">
              <div className="flex items-center gap-3 mb-4">
                <span className="material-symbols-outlined text-primary" data-icon="schedule">
                  schedule
                </span>
                <h4 className="font-headline-md text-headline-md text-primary">Durasi Garansi</h4>
              </div>
              <ul className="list-disc list-inside font-body-md text-body-md text-on-surface-variant space-y-2">
                <li>Service Hardware: 30 - 90 Hari</li>
                <li>Penggantian Sparepart: 3 - 6 Bulan (tergantung jenis)</li>
                <li>Instalasi Software: 14 Hari</li>
              </ul>
            </div>
            <div className="bg-surface p-6 border border-outline-variant rounded">
              <div className="flex items-center gap-3 mb-4">
                <span className="material-symbols-outlined text-primary" data-icon="fact_check">
                  fact_check
                </span>
                <h4 className="font-headline-md text-headline-md text-primary">Proses Klaim</h4>
              </div>
              <ol className="list-decimal list-inside font-body-md text-body-md text-on-surface-variant space-y-2">
                <li>Bawa perangkat dan nota perbaikan asli.</li>
                <li>Teknisi melakukan pengecekan ulang.</li>
                <li>Perbaikan dilakukan tanpa biaya tambahan jika masuk kriteria garansi.</li>
              </ol>
            </div>
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="py-16 md:py-20 px-4 md:px-8 lg:px-margin-desktop bg-primary-container text-on-primary-container text-center">
        <div className="max-w-container-max mx-auto">
          <h2 className="font-headline-lg text-headline-lg mb-4 md:font-headline-lg md:text-headline-lg font-headline-lg-mobile text-headline-lg-mobile">
            Butuh Bantuan Teknisi Kami?
          </h2>
          <p className="font-body-lg text-body-lg mb-8 max-w-2xl mx-auto">
            Jangan biarkan masalah perangkat mengganggu produktivitas Anda. Jadwalkan perbaikan sekarang.
          </p>
          <Link href="/booking-servis">
            <button className="bg-primary text-on-primary font-label-bold text-label-bold px-8 py-4 rounded hover:opacity-90 transition-opacity cursor-pointer">
              Booking Sekarang
            </button>
          </Link>
        </div>
      </section>
    </main>
  );
}
