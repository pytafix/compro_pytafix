...
export function FaqSection({ faqs }: { faqs: Array<Pick<Faq, "question" | "answer">> }) {
  const displayFaqs = faqs.length > 0 ? faqs : DEFAULT_FAQS.slice(0, 4);
  ...
  return (
    <motion.div variants={fadeUpVariant} className="bg-surface border border-outline-variant rounded-lg overflow-hidden">
      <button 
        ...
        aria-controls={`faq-${question.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}
      >
