    ...
    <>
      ...
      {isOpen && (
        <div className="md:hidden fixed inset-0 z-[60] bg-surface flex flex-col animate-in slide-in-from-bottom-4">
          ...
          <div className="flex flex-col py-6 px-6 gap-6 flex-1 overflow-y-auto">
            <div className="flex flex-col gap-4">
              {links.map((link) => (
                <Link 
                  key={link.name} 
                  href={link.href} 
                  className={`font-headline-sm text-2xl pb-4 border-b border-outline-variant/50 font-bold ${
            ...
            <Link 
              href="/booking-servis" 
              onClick={() => setIsOpen(false)} 
              className="w-full bg-primary text-on-primary font-label-bold text-lg px-6 py-4 rounded-xl shadow-md hover:shadow-lg transition-all text-center mt-2 cursor-pointer"
            >
              Booking Sekarang
            </Link>

            {/* Social Media Links */}
            <div className="flex items-center justify-center gap-4 mt-auto pt-8 pb-4">
