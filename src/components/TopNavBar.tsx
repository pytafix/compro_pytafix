...
export function TopNavBar() {
  ...
  return (
    <nav className="bg-surface fixed top-0 w-full z-50 border-b border-outline-variant shadow-sm">
      <div className="flex justify-between items-center h-20 px-4 md:px-8 lg:px-margin-desktop max-w-container-max mx-auto">
        ...
        <Link href="/booking-servis" className="hidden md:block">
          <button className="bg-primary text-on-primary font-label-bold text-label-bold px-4 lg:px-6 py-2.5 lg:py-3 rounded hover:opacity-90 hover:shadow-md transition-all cursor-pointer">
            Booking Sekarang
          </button>
        </Link>
      ...
      {isOpen && (
        <div className="md:hidden absolute top-20 left-0 w-full bg-surface border-b border-outline-variant shadow-lg flex flex-col items-center py-6 gap-6 animate-in slide-in-from-top-2">
          {links.map((link) => (
            <Link 
              key={link.name} 
              href={link.href} 
          ...
          <Link href="/booking-servis" onClick={() => setIsOpen(false)} className="w-full px-6 mt-4">
            <button className="w-full bg-primary text-on-primary font-label-bold text-label-bold px-6 py-3 rounded shadow-sm">
              Booking Sekarang
            </button>
          </Link>
        </div>
